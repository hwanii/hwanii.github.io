...
public class LoginService {
    ...
    switch (sep) {
      case "head_office":
        ...
        if (head != null && passwordEncoder.matches(userPw, head.getHdPw())) {
          ...
          return new LoginResult(token, head.getHdId());
        }
        break;
      ...
      case "agency":
        ...
        if (agency != null && passwordEncoder.matches(userPw, agency.getAgPw())) {
          String token = jwtUtil.generateToken(userId, sep);
          return new LoginResult(token, agency.getAgId());
        }
        break;
      ...
      case "logistic":
        ...
        if (logi != null && passwordEncoder.matches(userPw, logi.getLgPw())) {
          String token = jwtUtil.generateToken(userId, sep);
          return new LoginResult(token, logi.getLgId());
        }
        break;
    }
    return null;
  ...
  public static record LoginResult(String token, String userId) {}
}
